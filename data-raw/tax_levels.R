...
pars <- pars_raw %>%
  ...
  mutate(
    rank = if_else(rank == "division", "phylum", rank),
...
acc_pars <- pars3 %>%
  ...
  pivot_wider(
    names_from = upstream_rank,
    values_from = upstream_taxon
  ) %>%
  full_join(
    acc %>%
      distinct(
        acc_taxon_id, acc_taxon, acc_ss_sl, acc_rank,
...
acc_need_par <- acc_pars2 %>%
  ...
  mutate(
    itis_name = if_else(!is.na(acc_hybrid),
      acc_hybrid %>%
        str_replace_all("×", "X ") %>%
        str_replace_all("X  ", "X "),
      acc_taxon
...
acc_itis_ids <- name2taxid(acc_need_par$itis_name,
  db = "itis",
  out_type = "summary"
) %>%
...
(acc_wfo_ids <- name2taxid(itis_no_class$wfo_name,
  db = "wfo",
  out_type = "summary"
) %>%
...
(acc_gbif_ids <- name2taxid(wfo_no_class$gbif_name,
  db = "gbif",
  out_type = "summary"
) %>%
...
acc_db_pars <- acc_itis_pars2 %>%
  ...
  mutate(
    genus_rep = if_else(acc_rank == "genus", acc_taxon, genus_rep),
...
all_no_class <- gbif_no_class %>%
  ...
  select(
    acc_taxon_id, acc_taxon, acc_rank, species, genus, family, order,
...
all_no_class %>%
  group_by(acc_rank) %>%
  summarize(across(
    .cols = c(family, order, class, phylum, kingdom),
    .fns = ~ sum(is.na(.x))
  ))
...
acc_pars3 <- acc_pars2 %>%
  ...
  mutate(
    species = if_else(!is.na(species_rep), species_rep, species),
  ...
  mutate(
    family = case_when(
      acc_taxon == "Chamaesyce" ~ "Euphorbiaceae",
      TRUE ~ family
    ),
    order = case_when(
      acc_taxon == "Chamaesyce" ~ "Euphorbiales",
    ...
    class = case_when(
      acc_taxon == "Chamaesyce" ~ "Dicotyledoneae",
      order %in% c(
        "Caryophyllales", "Rosales",
    ...
    phylum = case_when(
      acc_taxon == "Chamaesyce" ~ "Anthophyta",
      order %in% c(
        "Caryophyllales", "Fabales",
        "Alismatales", "Rosales",
        "Celastrales", "Asterales",
        "Lamiales", "Solanales",
        "Rubiales", "Apiales", "Ericales",
...
acc_pars4 <- acc_pars3 %>%
  ...
  select(
    acc_taxon_id, acc_taxon, acc_ss_sl, acc_hybrid, acc_rank, species,
