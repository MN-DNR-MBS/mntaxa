...
accepted_mntaxa <- function(taxonomy_levels = FALSE,
  ...
  if (missing_accepted || missing_taxa || missing_taxonomy || missing_sources || missing_phys || missing_origin || missing_common || missing_cvals || missing_exclude) {
    load_mnnpc(accepted = missing_accepted,
  ...
  # format taxa names
  taxa2 <- taxa_mntaxa(taxonomy_levels = taxonomy_levels,
                       sources = sources)
  ...
  if (phys) {
    ...
    syn_phys2 <- syn_phys |>
      dplyr::left_join(phys_codes |>
                         dplyr::transmute(physiognomy_id = id,
                                          physiognomy,
                                          physcode = physiognomy_code)) |>
      dplyr::filter(!is.na(physiognomy) & is_acceptable == 1) |>
      dplyr::group_by(synonymy_id) |>
      dplyr::summarize(physiognomy = paste(sort(unique(physiognomy)), collapse = "/"),
  ...
  if (origin) {
    ...
    syn_or2 <- syn_or |>
      dplyr::left_join(origin_codes |>
                         dplyr::transmute(native_status_id = id,
                            native_status)) |>
      dplyr::transmute(synonymy_id = id,
                native_status) |>
      dplyr::filter(!is.na(native_status)) |>
      dplyr::group_by(synonymy_id) |>
      dplyr::summarize(native_status = paste(sort(unique(native_status)), collapse = "/"),
                .groups = "drop")
  ...
  if (common) {
    ...
    syn_comm2 <- syn_comm |>
      dplyr::group_by(synonymy_id) |>
      dplyr::summarize(common_name = paste(sort(unique(common_name)), collapse = "/"),
                .groups = "drop")
  ...
  if (exclude) {
    ...
    syn_exclude2 <- syn_exclude |>
      ...
      dplyr::left_join(exclude_codes |>
                         dplyr::transmute(excluded_reason_id = id,
